...
function inputLetter() {
    document.getElementsByTagName("html").item(0).addEventListener("keyup", function (event) {
        ...
        if (tentativo < possibilita && !win) {
            ...
            if (lettera.match(/^[a-z]$/)) {
            ...
            } else if (lettera == "enter" && inputCorrente.length == 5 && parole.includes(inputParola)) {
                ...
                if (inputParola == parola) {
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        for (let j = 0; j < 5; j++) {
                            if (!usate[j] && inputCorrente[i] === arrayParola[j]) {
                                risultato[i] = "presente";
                                usate[j] = true;
                                break;
                            }
                        }
